...
u8 read1(void);               //¶Á³ö±àºÅÎª1µÄRFIDÊý¾ÝPA9£¬PA10£¬×ó²àrfid¶Á¿¨Æ÷
u8 read2(void);               //¶Á³ö±àºÅÎª2µÄRFIDÊý¾ÝPA2£¬PA3£¬×ßÓÒ²à¶Á¿¨Æ÷
...
void TIM3_Init(void){
    ...
    TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;	  //¶¨Òå¸ñÊ½ÎªTIM_TimeBaseInitTypeDefµÄ½á¹¹ÌåµÄÃû×ÖÎªTIM_TimeBaseStructure  
  	...
  	TIM_TimeBaseStructure.TIM_Period =9999;		  // ÅäÖÃ¼ÆÊýãÐÖµÎª9999£¬³¬¹ýÊ±£¬×Ô¶¯ÇåÁã£¬²¢1ms´¥·¢ÖÐ¶Ï
...
int main(void)
{
  ...
  go_n_step(1);
	...
	xi=1;
	TIM_Cmd(TIM3,ENABLE);//¶¨Ê±Æ÷ÓÃÀ´×ªÍä
	while(ruo!=3){
		delay_nms(10);
	}
		brake();
...
void TIM3_IRQHandler(void){
  TIM_ClearITPendingBit(TIM3,TIM_IT_Update);
	...
	if(i==10){
		...
		f6=GPIO_ReadInputDataBit(GPIOG,GPIO_Pin_4);//5
		f11=GPIO_ReadInputDataBit(GPIOG,GPIO_Pin_7);//µÚÒ»´Î×ªÍäµÄÅÐ¶Ï
		i=0;
	} 
...
void nvic(void){
     ...
     NVIC_InitStructure.NVIC_IRQChannel = TIM4_IRQn; //¸ÃÖÐ¶ÏÎªTIM2Òç³ö¸üÐÂÖÐ¶Ï
     ...
     NVIC_Init(&NVIC_InitStructure);					   	//	³õÊ¼»¯
	 	 ...
	 	 NVIC_PriorityGroupConfig(NVIC_PriorityGroup_1);    //	   ½«ÓÅÏÈ¼¶·Ö×é·½Ê½ÅäÖÃÎªgroup1,ÓÐ2¸öÇÀÕ¼£¨´ò¶Ï£©ÓÅÏÈ¼¶£¬8¸öÏìÓ¦ÓÅÏÈ¼¶
     NVIC_InitStructure.NVIC_IRQChannel = TIM3_IRQn; //¸ÃÖÐ¶ÏÎªTIM2Òç³ö¸üÐÂÖÐ¶Ï
     NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;//´ò¶ÏÓÅÏÈ¼¶Îª1£¬ÔÚ¸Ã×éÖÐÎª½ÏµÍµÄ£¬0ÓÅÏÈ¼¶×î¸ß
     ...
     NVIC_InitStructure.NVIC_IRQChannel = USART3_IRQn; //TIM3	Òç³ö¸üÐÂÖÐ¶Ï
...
void USART3_Configuration(void){
	  USART_InitTypeDef USART_InitStructure; //´®¿ÚÉèÖÃ»Ö¸´Ä¬ÈÏ²ÎÊý
    ...
    USART_ITConfig(USART3, USART_IT_RXNE, ENABLE);  // Èô½ÓÊÕÊý¾Ý¼Ä´æÆ÷Âú£¬Ôò²úÉúÖÐ¶Ï
...
void USART3_IRQHandler(void){
	...
	{
		USART_ClearITPendingBit(USART3,USART_IT_RXNE);	//Çå³ý½ÓÊÜÖÐ¶Ï±êÖ¾Î»
		RxData=USART_ReceiveData(USART3);								//¶ÁÈ¡½ÓÊÜÊý¾Ý{
			switch(RxData)
			{
	...
	if(USART_GetITStatus(USART3,USART_IT_TXE)!=RESET)  //ÅÐ¶ÏÊÇ·ñÎª·¢ËÍÖÐ¶Ï
	{	
		USART_ClearITPendingBit(USART3,USART_IT_TXE);//Çå³ý·¢ËÍÖÐ¶Ï±êÖ¾Î»
		
	}
	if(USART_GetITStatus(USART3,USART_IT_TC)!=RESET)  //ÅÐ¶ÏÊÇ·ñÎª·¢ËÍÍê³ÉÖÐ¶Ï
	{
		USART_ClearITPendingBit(USART3,USART_IT_TC);	//Çå³ý·¢ËÍÍê³ÉÖÐ¶Ï±êÖ¾Î»
	}
...
u8 read1()
{
 	...
 	while(block[0]==0&&(times<10))
	{
		...
		t3=RFID_READ1(uid,block);//¶ÁÈ¡rfidµÚÒ»¿éÊý¾Ý
		times++;
	}	 
...
u8 read2()
{
 	...
 	while(block[0]==0&&(times<10))
	{
		...
		t=RFID_READ2(uid,block);//¶ÁÈ¡rfidµÚÒ»¿éÊý¾Ý
		times++;
	}	 
...
/*	setWheel(2,direction,40);
	while(sign!=1);
  if(sign==1){         //¶ÔµÚÒ»¸ùÖù×Ó½øÐÐ¼ì²â
			 USART_SendData(USART3, 'F');//ÒªÇó¶ÁÈ¡Ç°·½µÄrfid¶Á¿¨Æ÷
		   while(success!=1);          //µÈ´ýÊÕµ½·´À¡
		   delay_nms(20);
		   success=0;
		   if(mission==0){
